### MODULE ####
DisplayReponsesInput<-function(id) {
  ns <- NS(id)
  uiOutput(ns("reponsesUI"))
}

DisplayReponses<-function(input, output, session, the.df=nomo(), id.var="VARIABLE",label.var="QUESTION", type.var="Type",
  ...
  lapy<-lapply(X = 1:nrow(DF), FUN = function(i){
    if(input$sphinx_ou_word=="SPHINX"){
      inputId.i<-DF[i, id.var]
      label.i<-DF[i, "Libellé"]
      if(DF[i , type.var]=="titre"){
        h3(DF[i, "Libellé"])
      } else {
        if(DF[i, type.var]=="choix.multiple"){
          choices.i<-unlist(strsplit(x = as.character(DF$`Modalités`[i]), split = " ; "))
          checkboxGroupInput(inputId=inputId.i,
                             label=label.i,
                             choices=choices.i,
                             selected=choices.i[1], width = "100%")#choices.i[i], width = "100%")
        } else {
